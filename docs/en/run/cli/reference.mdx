...
**Compute unit selection** (via `--compute`) — SDK default is **`hybrid`** for llama.cpp (per-tensor HTP+CPU on Snapdragon) and **`npu`** for qairt:

```powershell
# llama.cpp — omit --compute for the SDK hybrid default
geniex infer unsloth/Qwen3.5-0.8B-GGUF
geniex infer unsloth/Qwen3.5-0.8B-GGUF --compute npu    # pin HTP0
geniex infer unsloth/Qwen3.5-0.8B-GGUF --compute gpu
geniex infer unsloth/Qwen3.5-0.8B-GGUF --compute cpu

# Qualcomm AI Hub Models only support NPU (non-npu aliases are coerced with a warning)
geniex infer ai-hub-models/Qwen3-4B
```

<Warning>Qualcomm AI Hub Models run on NPU only. Using `--compute cpu`, `gpu`, or `hybrid` is coerced to NPU with a warning.</Warning>
...
| `--nctx` | int | `16384` | Context window size (max input + output tokens). |
| `--kv-cache` | string | auto | KV cache type for both K and V (`f16`, `q8_0`, `q4_0`, …). Empty = auto: `q8_0` when `--nctx` ≥ 8192 (llama_cpp only). |
| `--cache-type-k` | string | — | Override KV cache type for K only (llama_cpp only). |
| `--cache-type-v` | string | — | Override KV cache type for V only (llama_cpp only). |
| `--max-history-turns` | int | `32` | Keep the last N user turns in multi-turn chat (system prompt kept; `0` = unlimited). |
| `--sliding-window` | — | `true` | (`qairt`) Evict oldest tokens on overflow instead of erroring. llama_cpp always context-shifts. |
| `--sliding-window-n-keep` | int | `0` | Tokens to keep anchored when sliding / context-shifting (`0` = plugin default 4). |
