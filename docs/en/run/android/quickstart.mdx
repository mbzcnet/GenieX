...
|--------------------|--------------------------------------------|
| `null`             | SDK default: **hybrid** (llama_cpp) or **npu** (qairt). |
| `"npu"`            | Pinned Hexagon HTP0 / QNN NPU.             |
| `"hybrid"`         | llama.cpp HTP+CPU scheduler (same as GGUF SDK default). |
| `"gpu"`            | Adreno GPU — OpenCL (Win) / Vulkan (Linux ARM64). |
| `"cpu"`            | Pure CPU. Works on any ARM64 chipset.      |

Qualcomm AI Engine Direct ignores this — non-npu aliases are coerced to NPU with a warning.
