...
```kotlin
val compute_unit: String?   // "cpu" | "gpu" | "npu" | "hybrid" | null
```

| Alias   | Effect                                                              |
|---------|---------------------------------------------------------------------|
| `null`  | SDK default — **`hybrid`** for `llama_cpp`, **`npu`** for `qairt`.  |
| `"npu"` | Pinned Hexagon HTP0 (`llama_cpp`) or QNN NPU (`qairt`).             |
| `"hybrid"` | llama.cpp per-tensor HTP+CPU scheduler (`llama_cpp` only). Same as SDK default for GGUF. |
| `"gpu"` | Adreno GPU — OpenCL (Windows) / Vulkan (Linux ARM64). `llama_cpp` only. |
| `"cpu"` | Pure CPU. Forces `nGpuLayers = 0`.                                  |

<Note>
Defaults are the same as CLI/Python (one SDK table). Qualcomm AI Engine Direct only supports NPU — passing `"cpu"`, `"gpu"`, or `"hybrid"` with a Qualcomm AI Hub Model logs a warning and falls back to NPU.
</Note>
...
data class ModelConfig(
    var nCtx:                  Int     = 8192,     // context size; 0 = model default
    var nThreads:              Int     = 8,
    ...
    var cacheTypeK:            String  = "",    // llama_cpp KV type; empty = auto (q8_0 when nCtx>=8192)
    var cacheTypeV:            String  = "",    // llama_cpp KV type; empty = auto
