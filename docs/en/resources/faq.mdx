...
<AccordionGroup>
  ...
  <Accordion title="Which runtime is the default?">
    One SDK table (`geniex_resolve_device`) for every surface: **`hybrid`** for llama.cpp (per-tensor HTP+CPU on Snapdragon Windows / Linux / Android), **`npu`** for `qairt`. Pass `--compute npu` / `device_map="npu"` for the deterministic pinned-HTP0 path.
  </Accordion>
