...
<AccordionGroup>
  ...
  <Accordion title="`--compute cpu` or `--compute gpu` errors with a Qualcomm AI Hub Model">
    Qualcomm AI Engine Direct is NPU-only. Omit `--compute` (SDK default for `qairt` is `npu`) or pass `--compute npu`. To run on CPU/GPU, switch to a GGUF model on the [llama.cpp runtime](/en/get-started/platforms#llamacpp).
  </Accordion>
</AccordionGroup>
