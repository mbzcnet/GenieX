...
<AccordionGroup>
  ...
  <Accordion title="`--compute cpu` 或 `--compute gpu` 在 Qualcomm AI Hub 模型上报错">
    Qualcomm AI Engine Direct 仅支持 NPU。省略 `--compute`（`qairt` 的 SDK 默认即为 `npu`）或显式传 `--compute npu`。如需在 CPU/GPU 上运行，改用 [llama.cpp 运行环境](/cn/get-started/platforms#llamacpp)的 GGUF 模型。
  </Accordion>
</AccordionGroup>
