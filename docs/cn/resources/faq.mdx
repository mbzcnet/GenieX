...
<AccordionGroup>
  ...
  <Accordion title="默认运行环境是哪个？">
    所有入口共用 SDK 一张表（`geniex_resolve_device`）：llama.cpp 默认 **`hybrid`**（骁龙 Windows / Linux / Android 上按张量 HTP+CPU），`qairt` 默认 **`npu`**。需要确定性钉死 HTP0 时传 `--compute npu` / `device_map="npu"`。
  </Accordion>
