...
| `"npu"`            | 钉死 Hexagon HTP0 / QNN NPU。               |
| `"hybrid"`         | llama.cpp HTP+CPU 调度（与 GGUF 的 SDK 默认相同）。 |
| `"gpu"`            | Adreno GPU——OpenCL（Windows）/ Vulkan（Linux ARM64）。 |
| `"cpu"`            | 纯 CPU。可在任意 ARM64 芯片组上运行。        |

Qualcomm AI Engine Direct 会忽略此设置——非 npu 别名会带警告强制到 NPU。
