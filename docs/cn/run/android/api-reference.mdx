...
| 别名    | 行为                                                                |
|---------|---------------------------------------------------------------------|
| `null`  | SDK 默认——`llama_cpp` 为 **`hybrid`**，`qairt` 为 **`npu`**。    |
| `"npu"` | 钉死 Hexagon HTP0（`llama_cpp`）或 QNN NPU（`qairt`）。             |
| `"hybrid"` | llama.cpp 按张量 HTP+CPU 调度（仅 `llama_cpp`）；与 GGUF 的 SDK 默认相同。 |
| `"gpu"` | Adreno GPU——OpenCL（Windows）/ Vulkan（Linux ARM64）。仅 `llama_cpp`。 |
| `"cpu"` | 纯 CPU。强制 `nGpuLayers = 0`。                                     |

<Note>
默认与 CLI/Python 相同（共用 SDK 一张表）。Qualcomm AI Engine Direct 仅支持 NPU——对 Qualcomm AI Hub Model 传 `"cpu"` / `"gpu"` / `"hybrid"` 仅告警并回退到 NPU。
</Note>
...
data class ModelConfig(
    ...
    var cacheTypeK:            String  = "",    // llama_cpp KV 类型；空 = 自动（nCtx>=8192 时 q8_0）
    var cacheTypeV:            String  = "",    // llama_cpp KV 类型；空 = 自动
...
<Note>
JNI 会根据 `compute_unit` 改写 `nGpuLayers`：`cpu` 强制为 0；`gpu` / `npu` / `hybrid` 保留传入值（`-1` = 全部层）。
</Note>
